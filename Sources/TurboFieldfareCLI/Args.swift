public struct Args: Equatable, Sendable {
    ...
    public static let allowedExpertCachePolicies = ["lfu", "lru"]
    public static let allowedRDAdvisePolicies = ["off", "default", "bounded", "adaptive"]
    ...
    public static func parse(_ argv: [String]) throws -> Args {
        ...
        while index < argv.count {
            ...
            case "--expert-cache-slots":
                let value = try takeValue(argv, &index, flag: flag)
                guard let parsed = Int(value), Args.allowedExpertCacheSlots.contains(parsed) else {
                    throw ArgsError.invalidValue(flag: flag, value: value)
                }
                expertCacheSlots = parsed
            case "--expert-cache-policy":
                let value = try takeValue(argv, &index, flag: flag)
                guard Args.allowedExpertCachePolicies.contains(value) else {
                    throw ArgsError.invalidValue(flag: flag, value: value)
                }
            ...
            case "--prefill-chunk-tokens":
                let value = try takeValue(argv, &index, flag: flag)
                guard let parsed = Int(value), Args.allowedPrefillChunkTokens.contains(parsed) else {
                    throw ArgsError.invalidValue(flag: flag, value: value)
                }
                prefillChunkTokens = parsed
            case "--rdadvise":
                let value = try takeValue(argv, &index, flag: flag)
                guard Args.allowedRDAdvisePolicies.contains(value) else {
                    throw ArgsError.invalidValue(flag: flag, value: value)
                }
